...
class HelpCommand(commands.Cog):
    ...
    async def send_help(self, ctx):
        ...
        embed.add_field(
            name="🎮 Опыт",
            value="`!опыт @user` — посмотреть опыт\n"
                  "`!опытгив @user кол-во причина` — выдать опыт (только особые роли)\n"
                  "`!снятьопыт @user кол-во причина` — снять опыт (только особые роли)\n"
                  "`!войсгив id_голосового_канала кол-во причина` — дать опыт всем в войсе",
            inline=False
        )
